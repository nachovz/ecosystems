...
class WP_Widget_Most_Viewed_Posts extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		$ec_post_type = $instance['ec_post_type'];
		
		$meta_key = apply_filters( 'baw_count_views_meta_key', '_count-views_' . $time . $date, $time, $date );
		global $bawpvc_options;
		
		if ( !isset($ec_post_type) || $ec_post_type == '' )
			$ec_post_type = $bawpvc_options['post_types'];	
		...
		$r = new WP_Query( array(	'posts_per_page' => $number, 
									...
									'post_type'=> apply_filters( 'baw_count_views_widget_post_types', $ec_post_type )
								) 
						);
		if ($r->have_posts()) :
		?>
		<?php echo $before_widget; ?>
		<?php $before_title = '<div class="bar"><span class="bar-line"></span><h5>';
			  $after_title = '</h5></div>';
		?>
		<?php if ( $title ) echo $before_title . strtoupper($title) . $after_title; ?>
		<ul>
			<?php $ind_count = 1; ?>
		<?php  while ($r->have_posts()) : $r->the_post(); ?>
			<?php $this_post_type = get_post_type( get_the_ID() ); ?>
		<?php
			$count = '';
			if( $instance['show'] ):
		...
		?>
		<li class="count-<?php echo $ind_count; ?>">
			<?php if($this_post_type == 'ec_applications') { ?>
			<span class="count-most-viewed-widget">#<?php echo $ind_count; ?></span>
			<?php } ?>
			<a href="<?php the_permalink() ?>" title="<?php the_title_attribute(); ?>" class="img-most-viewed-widget <?php if($this_post_type == 'ec_applications') echo 'lessmargin'; ?>">
				<?php echo ($this_post_type == 'ec_applications' || $this_post_type == 'post') ? the_post_thumbnail( 'wp-apps-thumb' ) : ''; ?>
			</a>
			<h4 <?php if($this_post_type != 'ec_applications') echo 'class="h4"'; ?> >
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['ec_post_type'] = $new_instance['ec_post_type'];
	...
	function form( $instance ) {
		...
		if( isset( $instance['order'] ) )
			...
			$order = 'DESC';
		$ec_post_type = isset($instance['ec_post_type']) ? $instance['ec_post_type'] : '';
?>
		...
		*/ ?>
		...
		<?php global $bawpvc_options; ?>
		<p><label for="<?php echo $this->get_field_id('ec_post_type'); ?>"><?php _e( 'Post Type', 'bawpvc' ); ?></label>
		<select id="<?php echo $this->get_field_id('ec_post_type'); ?>" name="<?php echo $this->get_field_name('ec_post_type'); ?>">
			
			<?php foreach( $bawpvc_options['post_types'] as $pt ) { ?>
				<option value="<?php echo $pt; ?>" <?php selected( $pt, $ec_post_type ); ?>><?php echo esc_html( $pt ); ?></option>
			<?php } ?>
